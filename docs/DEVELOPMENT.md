...
Individual source sizes are editable during preview and locked during recording. Guide margins are composition suggestions, and divider controls never appear in the saved video. Separate per-region files, capture across multiple displays, system audio, webcam overlays, and editing are not currently implemented.
...
xcrun swiftc -D VCAM_CAPTURE_VALIDATION -swift-version 5 \
  -target arm64-apple-macos15.0 \
  vcam/CaptureTypes.swift vcam/CapturePCM.swift vcam/CaptureMicrophone.swift \
  vcam/CaptureEngine.swift scripts/validate-composition.swift \
